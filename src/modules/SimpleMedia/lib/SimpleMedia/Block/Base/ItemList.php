...
class SimpleMedia_Block_Base_ItemList extends Zikula_Controller_AbstractBlock
{
    ...
    public function info()
    {
        ...
        return array('module'          => 'SimpleMedia',
                     'text_type'       => $this->__('SimpleMedia list view'),
                     'text_type_long'  => $this->__('Display list of SimpleMedia objects.'),
                     'allow_multiple'  => true,
                     'form_content'    => false,
                     'form_refresh'    => false,
                     'show_preview'    => true,
    ...
    public function display($blockinfo)
    {
        ...
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $properties = ModUtil::apiFunc('SimpleMedia', 'category', 'getAllProperties', array('ot' => $vars['objectType']));
        }
    
        // apply category filters
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            if (is_array($vars['catIds']) && count($vars['catIds']) > 0) {
                $categoryFiltersPerRegistry = ModUtil::apiFunc('SimpleMedia', 'category', 'buildFilterClauses', array('ot' => $objectType, 'catids' => $vars['catIds']));
    ...
    protected function getSortParam($vars, $repository)
    {
        ...
        if ($vars['sorting'] == 'newest') {
            $idFields = ModUtil::apiFunc('SimpleMedia', 'selection', 'getIdFields', array('ot' => $vars['objectType']));
            if (count($idFields) == 1) {
    ...
    public function update($blockinfo)
    {
        ...
        $vars['catIds'] = array($primaryRegistry => array());
        if (in_array($vars['objectType'], $this->categorisableObjectTypes)) {
            $vars['catIds'] = ModUtil::apiFunc('SimpleMedia', 'category', 'retrieveCategoriesFromRequest', array('ot' => $vars['objectType']));
        }
