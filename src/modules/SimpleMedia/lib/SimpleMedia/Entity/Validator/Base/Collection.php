...
class SimpleMedia_Entity_Validator_Base_Collection extends SimpleMedia_Validator
{
    ...
    public function validateAll()
    {
        ...
        if (!$this->isValidInteger('previewImage')) {
            $errorInfo['message'] = __f('Error! Field value may only contain digits (%s).', array('previewImage'), $dom);
            return $errorInfo;
        }
        if (!$this->isNumberNotLongerThan('previewImage', 11)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('previewImage', 11), $dom);
            return $errorInfo;
        }
        if (!$this->isValidInteger('sortValue')) {
            $errorInfo['message'] = __f('Error! Field value may only contain digits (%s).', array('sortValue'), $dom);
            return $errorInfo;
        }
        if (!$this->isNumberNotLongerThan('sortValue', 11)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('sortValue', 11), $dom);
            return $errorInfo;
        }
