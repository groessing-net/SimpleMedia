...
class SimpleMedia_Entity_Validator_Base_Medium extends SimpleMedia_Validator
{
    ...
    public function validateAll()
    {
        ...
        if (!$this->isStringNotLongerThan('theFile', 255)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('the file', 255), $dom);
            return $errorInfo;
        }
        if (!$this->isStringNotEmpty('theFile')) {
            $errorInfo['message'] = __f('Error! Field value must not be empty (%s).', array('the file'), $dom);
            return $errorInfo;
        }
        ...
        if (!$this->isStringNotLongerThan('zipcode', 15)) {
            $errorInfo['message'] = __f('Error! Length of field value must not be higher than %2$s (%1$s).', array('zipcode', 15), $dom);
            return $errorInfo;
        }
        if (!$this->isValidInteger('previewImage')) {
            $errorInfo['message'] = __f('Error! Field value may only contain digits (%s).', array('preview image'), $dom);
            return $errorInfo;
        }
        ...
        if ($this->entity['collection'] === null) {
            $errorInfo['message'] = __('Error! Choosing a collection is required.', $dom);
            return $errorInfo;
        }
