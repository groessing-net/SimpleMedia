...
// DEBUG: permission check aspect ends
    ...
    {
// DEBUG: permission check aspect starts
        $this->throwForbiddenUnless(SecurityUtil::checkPermission('SimpleMedia::', '::', ACCESS_READ));
// DEBUG: permission check aspect ends

        // parameter specifying which type of objects we are treating
        $objectType = (isset($args['ot']) && !empty($args['ot'])) ? $args['ot'] : $this->request->getGet()->filter('ot', 'medium', FILTER_SANITIZE_STRING);
        $utilArgs = array('controller' => 'user', 'action' => 'view');
        if (!in_array($objectType, SimpleMedia_Util_Controller::getObjectTypes('controllerAction', $utilArgs))) {
            $objectType = SimpleMedia_Util_Controller::getDefaultObjectType('controllerAction', $utilArgs);
        }
        ...
        if ($tpl == 'tree') {
            ...
            return SimpleMedia_Util_View::processTemplate($this->view, 'user', $objectType, 'view', $args);
        }
...
// DEBUG: permission check aspect ends
        ...
        // fetch and return the appropriate template
        return SimpleMedia_Util_View::processTemplate($this->view, 'user', $objectType, 'display', $args);
    }
}
