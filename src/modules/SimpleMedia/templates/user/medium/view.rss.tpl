{* purpose of this template: media rss feed in user area *}
...
{foreach item='medium' from=$items}
    <item>
        ...
        <link>{modurl modname='SimpleMedia' type='user' func='display' ot='medium' id=$medium.id slug=$medium.slug fqurl='1'}</link>
        <guid>{modurl modname='SimpleMedia' type='user' func='display' ot='medium' id=$medium.id slug=$medium.slug fqurl='1'}</guid>
