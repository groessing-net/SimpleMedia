...
{if $collection.root ne 1}
    {if !isset($allParents) || $allParents eq true}
        ...
        {if $allParents ne null && count($allParents) gt 0}
            ...
            {foreach item='node' from=$allParents}
                <li><a href="{modurl modname='SimpleMedia' type='admin' func='display' ot='collection' id=$node.id}" title="{$node.title|replace:'"':''}">{$node.title}</a></li>
            {/foreach}
            </ul>
        {/if}
    {/if}
    {if !isset($directParent) || $directParent eq true}
        ...
        {if $directParent ne null}
            ...
            <ul>
                <li><a href="{modurl modname='SimpleMedia' type='admin' func='display' ot='collection' id=$directParent.id}" title="{$directParent.title|replace:'"':''}">{$directParent.title}</a></li>
            </ul>
        {/if}
    {/if}
{/if}
