...
/**
 * Medium controller base class.
 */
class SimpleMedia_Controller_Base_Medium extends Zikula_AbstractController
{
    /**
     * Post initialise.
     *
    ...
    public function main()
    {
        ...
        $objectType = 'medium';
        $utilArgs = array('controller' => 'medium', 'action' => 'main');
        ...
        if ($legacyControllerType != 'admin') {
            ...
            $redirectUrl = ModUtil::url($this->name, 'medium', 'view', array('lct' => $legacyControllerType));
            
            return $this->redirect($redirectUrl);
        }
        
        // set caching id
        $this->view->setCacheId('medium_main');
        
        // return main template
        return $this->view->fetch('medium/main.tpl');
    }
    ...
    public function import()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'medium';
        $utilArgs = array('controller' => 'medium', 'action' => 'import');
        $permLevel = $legacyControllerType == 'admin' ? ACCESS_ADMIN : ACCESS_OVERVIEW;
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel), LogUtil::getErrorMsgPermission());
        /** TODO: custom logic */
        
        // return template
        return $this->view->fetch('medium/import.tpl');
    }
    ...
    public function view()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'medium';
        $utilArgs = array('controller' => 'medium', 'action' => 'view');
        ...
        $component = 'SimpleMedia:' . ucfirst($objectType) . ':';
        $instance = '::';
        ...
        $currentUrlObject = new Zikula_ModUrl($this->name, 'medium', 'view', ZLanguage::getLanguageCode(), $currentUrlArgs);
    ...
    public function display()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'medium';
        $utilArgs = array('controller' => 'medium', 'action' => 'display');
        $permLevel = $legacyControllerType == 'admin' ? ACCESS_ADMIN : ACCESS_READ;
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel), LogUtil::getErrorMsgPermission());
        $entityClass = $this->name . '_Entity_' . ucfirst($objectType);
        $repository = $this->entityManager->getRepository($entityClass);
        ...
        $currentUrlObject = new Zikula_ModUrl($this->name, 'medium', 'display', ZLanguage::getLanguageCode(), $currentUrlArgs);
    ...
    public function edit()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'medium';
        $utilArgs = array('controller' => 'medium', 'action' => 'edit');
        ...
        $handlerClass = $this->name . '_Form_Handler_Medium_Edit';
    ...
    public function delete()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'medium';
        $utilArgs = array('controller' => 'medium', 'action' => 'delete');
        ...
        if (!$deleteAllowed) {
            return LogUtil::registerError($this->__('Error! It is not allowed to delete this medium.'));
        }
        ...
        if ($confirmation && $deleteAllowed) {
            ...
            if (!$validators->hasErrors()) {
                ...
                // Let any hooks know that we have created, updated or deleted the medium
                $hookType = 'process_delete';
                $hook = new Zikula_ProcessHook($hookAreaPrefix . '.' . $hookType, $entity->createCompositeIdentifier());
                $this->notifyHooks($hook);
        
                // The medium was deleted, so we clear all cached pages this item.
                $cacheArgs = array('ot' => $objectType, 'item' => $entity);
                ModUtil::apiFunc($this->name, 'cache', 'clearItemCache', $cacheArgs);
        
                if ($legacyControllerType == 'admin') {
                    // redirect to the main page
                    $redirectUrl = ModUtil::url($this->name, 'medium', 'main', array('lct' => $legacyControllerType));
                } else {
    ...
    /**
     * This is a custom method.
     * Upload multiple media in one go from the frontend.
     *
     *
     * @return mixed Output.
     */
    public function multiUpload()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'medium';
        $utilArgs = array('controller' => 'medium', 'action' => 'multiUpload');
        $permLevel = $legacyControllerType == 'admin' ? ACCESS_ADMIN : ACCESS_OVERVIEW;
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel), LogUtil::getErrorMsgPermission());
        /** TODO: custom logic */
        
        // return template
        return $this->view->fetch('medium/multiUpload.tpl');
    }
    
    /**
     * This is a custom method.
     * Provide a list overview of multiple media for easy editing of media properties.
     *
     *
     * @return mixed Output.
     */
    public function editList()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'medium';
        $utilArgs = array('controller' => 'medium', 'action' => 'editList');
        $permLevel = $legacyControllerType == 'admin' ? ACCESS_ADMIN : ACCESS_OVERVIEW;
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel), LogUtil::getErrorMsgPermission());
        /** TODO: custom logic */
        
        // return template
        return $this->view->fetch('medium/editList.tpl');
    }
    ...
    public function handleSelectedEntries()
    {
        ...
        $redirectUrl = ModUtil::url($this->name, 'admin', 'main', array('ot' => 'medium'));
        ...
        foreach ($items as $itemid) {
            ...
            if ($action != 'delete') {
                ...
                $url = new Zikula_ModUrl($this->name, 'medium', 'display', ZLanguage::getLanguageCode(), $urlArgs);
            }
    ...
    public function handleInlineRedirect()
    {
        ...
        $this->view->display('medium/inlineRedirectHandler.tpl');
