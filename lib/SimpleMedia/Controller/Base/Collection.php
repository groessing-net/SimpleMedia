 ...
 */
class SimpleMedia_Controller_Base_Collection extends Zikula_AbstractController
{
    /**
     * Post initialise.
     *
    ...
    public function import()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'collection';
        $utilArgs = array('controller' => 'collection', 'action' => 'import');
        $permLevel = $legacyControllerType == 'admin' ? ACCESS_ADMIN : ACCESS_OVERVIEW;
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel), LogUtil::getErrorMsgPermission());
        /** TODO: custom logic */
        
        // return template
        return $this->view->fetch('collection/import.tpl');
    }
        ...
        if ($confirmation && $deleteAllowed) {
            ...
            if (!$validators->hasErrors()) {
                ...
                if ($legacyControllerType == 'admin') {
                    ...
                    $redirectUrl = ModUtil::url($this->name, 'collection', 'main', array('lct' => $legacyControllerType));
                } else {
    ...
    public function multiUpload()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'collection';
        $utilArgs = array('controller' => 'collection', 'action' => 'multiUpload');
        $permLevel = $legacyControllerType == 'admin' ? ACCESS_ADMIN : ACCESS_OVERVIEW;
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel), LogUtil::getErrorMsgPermission());
        /** TODO: custom logic */
        
        // return template
        return $this->view->fetch('collection/multiUpload.tpl');
    }
    ...
    public function editList()
    {
        ...
        // parameter specifying which type of objects we are treating
        $objectType = 'collection';
        $utilArgs = array('controller' => 'collection', 'action' => 'editList');
        $permLevel = $legacyControllerType == 'admin' ? ACCESS_ADMIN : ACCESS_OVERVIEW;
        $this->throwForbiddenUnless(SecurityUtil::checkPermission($this->name . ':' . ucfirst($objectType) . ':', '::', $permLevel), LogUtil::getErrorMsgPermission());
        /** TODO: custom logic */
        
        // return template
        return $this->view->fetch('collection/editList.tpl');
    }
