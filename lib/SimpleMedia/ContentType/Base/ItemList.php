...
class SimpleMedia_ContentType_Base_ItemList extends Content_AbstractContentType
{
    ...
    public function loadData(&$data)
    {
        ...
        if (in_array($this->objectType, $this->categorisableObjectTypes)) {
            ...
            $this->catRegistries = ModUtil::apiFunc('SimpleMedia', 'category', 'getAllPropertiesWithMainCat', array('ot' => $this->objectType, 'arraykey' => $idFields[0]));
            $this->catProperties = ModUtil::apiFunc('SimpleMedia', 'category', 'getAllProperties', array('ot' => $this->objectType));
        }
    
        if (!isset($data['catIds'])) {
            $primaryRegistry = ModUtil::apiFunc('SimpleMedia', 'category', 'getPrimaryProperty', array('ot' => $this->objectType));
    ...
    public function display()
    {
        ...
        $entityClass = 'SimpleMedia_Entity_' . ucfirst($this->objectType);
        $serviceManager = ServiceUtil::getManager();
        ...
        // set cache id
        $component = 'SimpleMedia:' . ucfirst($this->objectType) . ':';
        $instance = '::';
        ...
        if (in_array($this->objectType, $this->categorisableObjectTypes)) {
            if (is_array($this->catIds) && count($this->catIds) > 0) {
                $qb = ModUtil::apiFunc('SimpleMedia', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => $this->objectType, 'catids' => $this->catIds));
            }
        }
