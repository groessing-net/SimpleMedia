...
require_once('modules/SimpleMedia/lib/vendor/getID3/getid3/getid3.php');
...
class SimpleMedia_UploadHandler extends SimpleMedia_Base_UploadHandler
{
     ...
     * OVERRIDE: adding extra meta data enquiry and returning that in result
     *
    ...
    public function performFileUpload($objectType, $fileData, $fieldName)
    {
        ...
        // CHANGED MOST CODE (move_uploaded_file => rename)
        
        if (!rename($fileData[$fieldName]['tmp_name'], $basePath . $fileName)) {
        ...
        // Initialize getID3 engine for metatags extraction
        // Analyze file and store returned data results
        $getID3 = new getID3;
        $metaTags = $getID3->analyze($basePath . $fileName);
        // only store a subset for now
        $result['metaTags'] = array(
            'jpg' => $metaTags['jpg'],
            'tags' => $metaTags['tags'],
            'xmp' => $metaTags['xmp'],
            'video' => $metaTags['video'],
            'mpeg' => $metaTags['mpeg']
        );
    ...
    public function readMetaDataForFile($fileName, $filePath)
    {
        ...
        if (empty($fileName) || $meta['isImage']) {
            return $meta;
        }
        ...
        // extract only EXIF and IPTC for jpeg and tif files
        if ($meta['imagetype'] == SimpleMedia_Util_Image::IMAGETYPE_JPEG ||
            ...
            $meta['exif'] = DataUtil::convertToUTF8(exif_read_data($filePath, 0, true));
            if ($meta['exif']) {
            ...
            if (isset($imageInfo) && is_array($imageInfo)) {
                // now extract IPTC information and convert to UTF-8 to prevent corruption of data.
                $meta['iptc'] = DataUtil::convertToUTF8(iptcparse($imageInfo["APP13"]));
