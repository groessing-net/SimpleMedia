 ...
 */
class SimpleMedia_Entity_Repository_Base_Medium extends EntityRepository
{
    /**
    ...
    public function getAdditionalTemplateParameters($context = '', $args = array())
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if (in_array($args['action'], array('main', 'view'))) {
                ...
                $templateParameters['workflowStateItems'] = $listHelper->getEntries('medium', 'workflowState');
                $templateParameters['mediaTypeItems'] = $listHelper->getEntries('medium', 'mediaType');
            }
            ...
            $objectType = 'medium';
            $templateParameters[$objectType . 'ThumbPresetTheFile'] = $imageHelper->getPreset($objectType, 'theFile', $context, $args);
            if (in_array($args['action'], array('display', 'view'))) {
                // use separate preset for images in related items
                $templateParameters['relationThumbPreset'] = $imageHelper->getCustomPreset('', '', 'SimpleMedia_relateditem', $context, $args);
            }
        }
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['catIdList'] = ModUtil::apiFunc('SimpleMedia', 'category', 'retrieveCategoriesFromRequest', array('ot' => 'medium', 'source' => 'GET'));
        ...
        $parameters['mediaType'] = isset($this->controllerArguments['mediaType']) ? $this->controllerArguments['mediaType'] : FormUtil::getPassedValue('mediaType', '', 'GET');
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('SimpleMedia_Entity_Medium', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('SimpleMedia_Entity_Medium', 'tbl')
     ...
     *
     * @return array|SimpleMedia_Entity_Medium retrieved data array or SimpleMedia_Entity_Medium instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     *
     * @return SimpleMedia_Entity_Medium retrieved instance of SimpleMedia_Entity_Medium
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                SessionUtil::setVar('SimpleMediaMediaCurrentPage', $page);
            } else {
                // restore current page from session
                $page = SessionUtil::getVar('SimpleMediaMediaCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('SimpleMedia', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'medium', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
    ...
    protected function applyDefaultFilters(QueryBuilder $qb, $parameters = array())
    {
        ...
        if ($currentLegacyControllerType == 'admin' && $currentModule == 'SimpleMedia') {
            return $qb;
        }
    
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved media only
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('SimpleMedia_Entity_Medium', 'tbl');
    ...
    protected function genericBaseQueryAddWhere(QueryBuilder $qb, $where = '')
    {
        ...
        $showOnlyOwnEntries = (int) FormUtil::getPassedValue('own', ModUtil::getVar('SimpleMedia', 'showOnlyOwnEntries', 0), 'GETPOST');
