 ...
 */
class SimpleMedia_Entity_Repository_Base_Collection extends NestedTreeRepository
{
    /**
    ...
    protected function getViewQuickNavParameters($context = '', $args = array())
    {
        ...
        $parameters = array();
        $parameters['catIdList'] = ModUtil::apiFunc('SimpleMedia', 'category', 'retrieveCategoriesFromRequest', array('ot' => 'collection', 'source' => 'GET'));
    ...
    public function truncateTable()
    {
        ...
        $qb->delete('SimpleMedia_Entity_Collection', 'tbl');
        $query = $qb->getQuery();
    ...
    public function updateCreator($userId, $newUserId)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->update('SimpleMedia_Entity_Collection', 'tbl')
     ...
     *
     * @return array|SimpleMedia_Entity_Collection retrieved data array or SimpleMedia_Entity_Collection instance
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
     ...
     * @param boolean $slimMode If activated only some basic fields are selected without using any joins (optional) (default=false).
     *
     * @return ArrayCollection collection containing retrieved SimpleMedia_Entity_Collection instances
     *
     * @throws InvalidArgumentException Thrown if invalid parameters are received
     */
    ...
    public function selectWherePaginated($where = '', $orderBy = '', $currentPage = 1, $resultsPerPage = 25, $useJoins = true, $slimMode = false)
    {
        ...
        if (!$hasFilters) {
            if ($page > 1 || isset($_GET['pos'])) {
                ...
                SessionUtil::setVar('SimpleMediaCollectionsCurrentPage', $page);
            } else {
                // restore current page from session
                $page = SessionUtil::getVar('SimpleMediaCollectionsCurrentPage', 1);
    ...
    public function addCommonViewFilters(QueryBuilder $qb)
    {
        ...
        foreach ($parameters as $k => $v) {
            if ($k == 'catId') {
                 ...
                 */
                $qb = ModUtil::apiFunc('SimpleMedia', 'category', 'buildFilterClauses', array('qb' => $qb, 'ot' => 'collection', 'catids' => $v));
            } elseif ($k == 'searchterm') {
                // quick search
                if (!empty($v)) {
    ...
    protected function getCountQuery($where = '', $useJoins = true)
    {
        ...
        $qb = $this->getEntityManager()->createQueryBuilder();
        $qb->select($selection)
           ->from('SimpleMedia_Entity_Collection', 'tbl');
    ...
    {
        $qb->leftJoin('tbl.media', 'tblMedia');
