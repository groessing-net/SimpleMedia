 ...
 */
class SimpleMedia_Base_RouterFacade
{
    /**
    ...
    protected function initUrlRoutes()
    {
        ...
        $isDefaultModule = (System::getVar('shorturlsdefaultmodule', '') == 'SimpleMedia');
    
        $defaults = array();
        $modulePrefix = '';
        if (!$isDefaultModule) {
            $defaults['module'] = 'SimpleMedia';
            $modulePrefix = ':module/';
        }
    ...
    public function getFormattedSlug($objectType, $func, $args, $itemid)
    {
        ...
        switch ($objectType) {
            case 'medium':
                $item = ModUtil::apiFunc('SimpleMedia', 'selection', 'getEntity', array('ot' => $objectType, 'id' => $itemid, 'slimMode' => true));
                $slug = $item['slug'];
                        break;
            case 'collection':
                $slug = $itemid;
