{* purpose of this template: media display view *}
...
<div class="simplemedia-medium simplemedia-display">
    ...
    {if $lct eq 'admin'}
        <div class="z-admin-content-pagetitle">
            ...
            <h3>{$templateTitle|notifyfilters:'simplemedia.filter_hooks.media.filter'}{icon id="itemActions`$medium.id`Trigger" type='options' size='extrasmall' __alt='Actions' class='z-pointer z-hide'}
            </h3>
        </div>
    {else}
        <h2>{$templateTitle|notifyfilters:'simplemedia.filter_hooks.media.filter'}{icon id="itemActions`$medium.id`Trigger" type='options' size='extrasmall' __alt='Actions' class='z-pointer z-hide'}
        </h2>
    {/if}
    ...
    <dl>
        ...
        {if isset($medium.Collection) && $medium.Collection ne null}
          ...
          {/strip}</a>
          <a id="collectionItem{$medium.Collection.id}Display" href="{modurl modname='SimpleMedia' type=$lct func='display' ot='collection'  id=$medium.Collection.id' theme='Printer' forcelongurl=true}" title="{gt text='Open quick view window'}" class="z-hide">{icon type='view' size='extrasmall' __alt='Quick view'}</a>
    ...
    {include file='helper/include_attributes_display.tpl' obj=$medium}
    {include file='helper/include_categories_display.tpl' obj=$medium}
    {include file='helper/include_metadata_display.tpl' obj=$medium}
    {include file='helper/include_standardfields_display.tpl' obj=$medium}
    ...
    {if !isset($smarty.get.theme) || $smarty.get.theme ne 'Printer'}
        ...
        {if count($medium._actions) gt 0}
            ...
            /* <![CDATA[ */
                document.observe('dom:loaded', function() {
                    simmedInitItemActions('medium', 'display', 'itemActions{{$medium.id}}');
                });
            /* ]]> */
            </script>
        {/if}
