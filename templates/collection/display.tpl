...
<div class="simplemedia-collection simplemedia-display with-rightbox">
    ...
    {if isset($collection.media) && $collection.media ne null}
        {include file='medium/include_displayItemGrid.tpl' items=$collection.media}
    {/if}
    ...
    {if !isset($directChildren) || $directChildren eq true}
        ...
        {if $directChildren ne null && count($directChildren) gt 0}
            {include file='collection/view_items.tpl' collections=$directChildren gridLevel=$collection.lvl+1}
        {/if}
    {/if}
