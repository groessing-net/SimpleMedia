...
{if $collection.lvl gt 0}
    {if !isset($allParents) || $allParents eq true}
        ...
        {if $allParents ne null && count($allParents) gt 0}
            ...
            {foreach item='node' from=$allParents}
                <li><a href="{modurl modname='SimpleMedia' type=$lct func='display' ot='collection'  id=$node.id}" title="{$node->getTitleFromDisplayPattern()|replace:'"':''}">{$node->getTitleFromDisplayPattern()}</a></li>
            {/foreach}
            </ul>
        {/if}
    {/if}
    {if !isset($directParent) || $directParent eq true}
        ...
        {if $directParent ne null}
            ...
            <ul>
                <li><a href="{modurl modname='SimpleMedia' type=$lct func='display' ot='collection'  id=$directParent.id}" title="{$directParent->getTitleFromDisplayPattern()|replace:'"':''}">{$directParent->getTitleFromDisplayPattern()}</a></li>
            </ul>
        {/if}
    {/if}
{/if}
