...
{if $properties ne null && is_array($properties)}
    ...
    {foreach key='propertyName' item='propertyId' from=$properties}
        <p>
            {modapifunc modname='SimpleMedia' type='category' func='hasMultipleSelection' ot='medium' registry=$propertyName assign='hasMultiSelection'}
            ...
            &nbsp;
            {selector_category name="`$baseID`_`$categorySelectorName``$propertyName`" field='id' selectedValue=$catIds.$propertyName categoryRegistryModule='SimpleMedia' categoryRegistryTable=$objectType categoryRegistryProperty=$propertyName defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize}
            <br{$break} />
        </p>
    {/foreach}
    {/nocache}
{/if}
<p>
    <label for="{$baseID}Id"{$leftSide}>{gt text='Medium'}:</label>
    <select id="{$baseID}Id" name="id"{$rightSide}>
        {foreach item='medium' from=$items}
            <option value="{$medium.id}"{if $selectedId eq $medium.id} selected="selected"{/if}>{$medium->getTitleFromDisplayPattern()}</option>
        {foreachelse}
...
<p>
    ...
    <input type="button" id="simpleMediaSearchGo" name="gosearch" value="{gt text='Filter'}" />
    <br{$break} />
</p>
<br />
<br />
