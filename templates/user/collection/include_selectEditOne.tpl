...
{/if}
    ...
    {if $displayMode eq 'dropdown'}
        {formlabel for=$alias __text='Choose collection' mandatorysym='1'}
            {simplemediaRelationSelectorList group=$group id=$alias aliasReverse=$aliasReverse mandatory=$mandatory __title='Choose the collection' selectionMode='single' objectType='collection' linkingItem=$linkingItem}
    {elseif $displayMode eq 'autocomplete'}
        {assign var='createLink' value=''}
        {if $allowEditing eq true}
            {modurl modname='SimpleMedia' type='user' func='edit' ot='collection' forcelongurl=true assign='createLink'}
        {/if}
        {simplemediaRelationSelectorAutoComplete group=$group id=$alias aliasReverse=$aliasReverse mandatory=$mandatory __title='Choose the collection' selectionMode='single' objectType='collection' linkingItem=$linkingItem idPrefix=$idPrefix createLink=$createLink withImage=false}
        <div class="simplemedia-relation-leftside">
