...
{strip}"{gt text='Title'}";"{gt text='Description'}";"{gt text='Preview image'}";"{gt text='Sort value'}";"{gt text='Views count'}";"{gt text='Workflow state'}"
;"{gt text='Media'}"
{/strip}
{foreach item='collection' from=$items}
{strip}
    "{$collection.title}";"{$collection.description}";"{$collection.previewImage}";"{$collection.sortValue}";"{$collection.viewsCount}";"{$item.workflowState|simplemediaObjectState:false|lower}"
    ;"
